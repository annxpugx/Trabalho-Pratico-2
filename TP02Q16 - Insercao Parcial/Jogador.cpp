...
void insercao(Jogador *Jogadores, int n, int k, int *comp, int *mov) {
    Jogador tmp = Jogador();
    for (int i = 1; i < n; i++) {
        tmp.clone(Jogadores[i]);
        int j = (i < k)? i - 1: k - 1;
        while (((j >= 0) && (Jogadores[j].getAnoNascimento() > tmp.getAnoNascimento()) || ((j >= 0) && Jogadores[j].getAnoNascimento() == tmp.getAnoNascimento() && (strcmp(Jogadores[j].getNome(), tmp.getNome()) > 0)))){
            (*comp)+=3;
            (*mov)++;
...
int main(){
    ...
    int mov = 0;
    int k = 10;
    insercao(_Jogadores, numEntrada_id, k, &comp, &mov);
    ...
    FILE *matricula;
    matricula = fopen("694370_insercaoParcial.txt", "w");
